...
// Mirrors ResultsScreen layout exactly — data comes from a saved Supabase session
// instead of useStyleReport / tryonSession. No API calls are made here.
...
import type { SavedTryonSession } from "@/lib/tryonStore";

// ── Translations (matches Results.tsx T) ─────────────────────────────────────
const T = {
  en: {
...
// ── Sub-components (identical to Results.tsx) ─────────────────────────────────
...
function ConfidenceBadge({ report, lang }: { report: StyleReport | null; lang: Language }) {
  ...
  return (
    <span
      ...
      style={{ color }}
    >
      · {T[lang].confidence[report.confidence]}
    </span>
  );
}
  ...
  return (
    <div className="py-[14px]" style={{ borderTop: "1px solid rgba(20,16,22,0.08)" }}>
      <div className="flex items-center gap-[12px] mb-[8px]">
        ...
        <span className="font-[family-name:var(--font-bodoni)] text-[18px] text-[#141016]">
          {score}<span className="text-[12px] text-[#9A9298]">/{max}</span>
        </span>
      </div>
...
interface SavedResultDetailProps {
  session: SavedTryonSession;
  onBack: () => void;
}

export function SavedResultDetail({ session, onBack }: SavedResultDetailProps) {
  ...
  const resultUrl  = session.result_image_url;
  const dualReport = session.style_report as DualReport | null;
  const report     = dualReport?.[language] ?? null;
  const score      = report?.score ?? null;

  // Data is already loaded — never "loading"
  ...
  // ── Image panel (identical to ResultsScreen) ────────────────────────────────
  const imagePanel = (
    <div className="relative rounded-[20px] overflow-hidden border border-[rgba(20,16,22,0.08)] bg-white h-[380px] md:h-full md:min-h-[560px]">
      ...
      <button
        ...
        disabled={!report}
      >
        <ScoreCircle score={score} loading={false} />
      </button>
    </div>
  );

  // ── Verdict content (identical to ResultsScreen) ─────────────────────────────
    ...
    <>
        ...
        <div>
          ...
          <div className="flex flex-col">
            ...
            <button
              ...
              disabled={!report}
              className="flex justify-between items-center py-[20px] border-none bg-transparent cursor-pointer text-left w-full disabled:cursor-default"
            ...
            {/* Styling Tips */}
            <div className="py-[20px]" style={{ borderTop: "1px solid rgba(20,16,22,0.08)" }}>
              <span className="font-[family-name:var(--font-mono)] text-[10px] tracking-[0.14em] uppercase text-[#9A9298]">
                {t.tips}
              </span>
              {tips.length > 0 ? (
      ...
      <div className="mt-[16px] pb-[calc(22px+env(safe-area-inset-bottom))] md:pb-[22px] flex flex-col gap-[10px]">
        <div className="flex gap-[10px]">
          ...
          {/* Already saved — show disabled "Saved" state */}
          <button
            disabled
            className="flex-1 py-[16px] border border-[rgba(20,16,22,0.14)] rounded-full bg-white font-[family-name:var(--font-grotesk)] font-medium text-[14px] cursor-not-allowed opacity-50 whitespace-nowrap"
            style={{ color: "#6B6470" }}
          >
            {t.saved}
          </button>
        ...
        <div className="flex gap-[10px] items-center">
          ...
          <MiniTab
            ...
            onChange={(v) => setVariant(v.toLowerCase() as "a" | "b")}
          />
        </div>
  ...
  // ── Bottom sheets (identical to ResultsScreen) ───────────────────────────────
  const scoreSheet = (
      ...
      {report && personalColor && (
        <div className="flex flex-col gap-[18px]">
          <div>
            ...
            <div className="font-[family-name:var(--font-bodoni)] text-[28px] text-[#141016] mb-[8px]">{personalColor.color_type}</div>
          </div>
          <div style={{ borderTop: "1px solid rgba(20,16,22,0.08)", paddingTop: "18px" }}>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:hidden">
        {/* Back + report header */}
        <div className="px-[20px] pt-[14px] flex items-center justify-between">
          <button
            onClick={onBack}
            className="flex items-center gap-[6px] border-none bg-transparent p-0 cursor-pointer font-[family-name:var(--font-mono)] text-[10px] tracking-[0.12em] uppercase text-[#9A9298]"
          >
            {t.backToWardrobe}
          </button>
          <LanguageToggle language={language} setLanguage={setLanguage} />
        </div>
        <div className="px-[20px] pt-[6px] flex items-center gap-[10px]">
          <span className="font-[family-name:var(--font-mono)] text-[11px] tracking-[0.16em] uppercase text-[#9A9298]">
            {t.reportLabel}
          </span>
          <ConfidenceBadge report={report} lang={language} />
        </div>
        ...
        <div className="md:sticky md:top-[40px] md:self-start">
          <div className="flex items-center justify-between mb-[16px]">
            <div className="flex items-center gap-[10px]">
              ...
              <span className="text-[#C8C2CC]">·</span>
              <span className="font-[family-name:var(--font-mono)] text-[11px] tracking-[0.16em] uppercase text-[#9A9298]">
                {t.reportLabel}
              </span>
              <ConfidenceBadge report={report} lang={language} />
